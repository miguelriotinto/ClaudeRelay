...
| Package structure | Monorepo with shared ClaudeRelayKit | Shared types across server, CLI, iOS |
...
├── Package.swift
├── Sources/
│   ├── ClaudeRelayKit/           # Shared: protocol types, models, crypto
...
│   │   └── main.swift
│   ├── ClaudeRelayCLI/           # CLI binary (claude-relay)
...
└── ClaudeRelayApp/               # iOS Xcode project
    └── (SwiftUI app importing ClaudeRelayClient)
```

### 3.2 macOS Service (ClaudeRelayServer)
...
```c
// Sources/ClaudeRelayKit/PTY/pty_shim.c
#include <util.h>
...
### Phase 1: Foundation
- Package structure + shared types (ClaudeRelayKit)
- Protocol message types (Codable enums)
